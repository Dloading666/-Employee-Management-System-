...
import edu.cqie.eams.domain.LeaveRecord;
import edu.cqie.eams.service.LeaveRecordService;
...
public class LeaveRecordController {
    ...
    @ResponseBody
    public edu.cqie.eams.comm.RetJson<String> approve(@PathVariable("id") Long id, @RequestParam(required=false) String remark, jakarta.servlet.http.HttpSession session) {
        try {
            Boolean isAdmin = (Boolean) session.getAttribute("isAdmin");
            if (isAdmin == null || !isAdmin) {
                return edu.cqie.eams.comm.RetJson.error("无权限");
            }
            if(remark==null||remark.isEmpty()) remark="同意";
            edu.cqie.eams.domain.User admin = (edu.cqie.eams.domain.User) session.getAttribute("currentUser");
            edu.cqie.eams.domain.LeaveRecord rec = leaveRecordService.getById(id);
            if (rec == null) return edu.cqie.eams.comm.RetJson.error("记录不存在");
            leaveRecordService.updateStatus(id,2,admin.getId(),new java.util.Date(),remark);
            return edu.cqie.eams.comm.RetJson.success("已批准");
        }catch(Exception e){
            e.printStackTrace();
            return edu.cqie.eams.comm.RetJson.error("审批异常:"+e.getMessage());
        }
    }
    @PostMapping("/reject/{id}")
    @ResponseBody
    public edu.cqie.eams.comm.RetJson<String> reject(@PathVariable("id") Long id, @RequestParam(required=false) String remark, jakarta.servlet.http.HttpSession session) {
        try {
            Boolean isAdmin = (Boolean) session.getAttribute("isAdmin");
            if (isAdmin == null || !isAdmin) {
                return edu.cqie.eams.comm.RetJson.error("无权限");
            }
            if(remark==null||remark.isEmpty()) return edu.cqie.eams.comm.RetJson.error("请填写驳回原因");
            edu.cqie.eams.domain.User admin = (edu.cqie.eams.domain.User) session.getAttribute("currentUser");
            edu.cqie.eams.domain.LeaveRecord rec = leaveRecordService.getById(id);
            if (rec == null) return edu.cqie.eams.comm.RetJson.error("记录不存在");
            leaveRecordService.updateStatus(id,3,admin.getId(),new java.util.Date(),remark);
            return edu.cqie.eams.comm.RetJson.success("已驳回");
        }catch(Exception e){
            e.printStackTrace();
            return edu.cqie.eams.comm.RetJson.error("驳回异常:"+e.getMessage());
        }
    }
    @PostMapping("/status/{id}")
    @ResponseBody
    public edu.cqie.eams.comm.RetJson<String> updateStatus(@PathVariable Long id,
                                                           @RequestParam Integer status,
                                                           @RequestParam(required=false) String remark,
                                                           jakarta.servlet.http.HttpSession session){
        Boolean isAdmin=(Boolean)session.getAttribute("isAdmin");
        if(isAdmin==null||!isAdmin){return edu.cqie.eams.comm.RetJson.error("无权限");}
        if(status==null||status<0){return edu.cqie.eams.comm.RetJson.error("非法状态");}
        edu.cqie.eams.domain.User admin=(edu.cqie.eams.domain.User)session.getAttribute("currentUser");
        try{
            leaveRecordService.updateStatus(id,status,admin.getId(),new java.util.Date(),remark);
            return edu.cqie.eams.comm.RetJson.success("状态已更新");
        }catch(Exception e){e.printStackTrace();return edu.cqie.eams.comm.RetJson.error("更新失败"+e.getMessage());}
    }
    @InitBinder
    ...
    @ResponseBody
    public edu.cqie.eams.comm.RetJson<String> batchStatus(@RequestBody java.util.List<StatusDTO> list,
                                                          jakarta.servlet.http.HttpSession session){
        Boolean isAdmin=(Boolean)session.getAttribute("isAdmin");
        if(isAdmin==null||!isAdmin) return edu.cqie.eams.comm.RetJson.error("无权限");
        edu.cqie.eams.domain.User admin=(edu.cqie.eams.domain.User)session.getAttribute("currentUser");
        ...
        return edu.cqie.eams.comm.RetJson.success("已保存 "+success+" 条");
    }
} 